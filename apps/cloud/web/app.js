(function(){
  ...
  document.getElementById('settings-form').addEventListener('submit',function(e){e.preventDefault();fetch('/api/v1/admin/settings',{method:'PUT',headers:Object.assign({'Content-Type':'application/json'},authHeaders()),body:JSON.stringify({display_name:document.getElementById('display-name').value,notice:document.getElementById('notice').value})}).then(function(r){return r.json();}).then(function(r){document.getElementById('settings-message').textContent=r.ok?'已保存':'保存失败：'+(r.error&&r.error.message||'未知错误');}).catch(function(){document.getElementById('settings-message').textContent='保存失败，请检查网络';});});
  load();
}());
